<img src="../documentos/assets/assets_readme/logoInteli.png">
...
- [2. Visão Geral da Aplicação Web](#2-visão-geral-da-aplicação-web)
  - [2.1. Escopo do Projeto](#21-escopo-do-projeto)
    ...
    - [2.1.3. Análise SWOT](#213-análise-swot)
      ...
      - [2. Fraquezas (Weaknesses):](#2-fraquezas-weaknesses)
...
- [5. Testes](#5-testes)
  ...
  - [5.2. Testes de usabilidade ](#52-testes-de-usabilidade)
    - [5.2.1 Registros de testes e melhorias](#521-registros-de-testes-e-melhorias)
...
&nbsp;&nbsp;&nbsp;&nbsp;Vários fatores podem afetar o poder de barganha dos fornecedores. Por exemplo, a disponibilidade de alternativas, a qualidade e a especialização do fornecedor, e a importância do fornecedor para a universidade. Se a *Blackboard Inc.* decidir aumentar seus preços, a Zuyd University pode enfrentar o desafio de absorver os custos adicionais ou procurar uma alternativa, o que pode ser difícil e caro.
...
&nbsp;&nbsp;&nbsp;&nbsp;Uma ameaça específica para a Universidade Zuyd pode ser a diminuição do financiamento governamental para o ensino superior. Isso poderia afetar a capacidade da universidade de oferecer certos programas ou bolsas de estudo. Outra ameaça pode ser a diminuição do número de estudantes do ensino médio, o que poderia levar a uma diminuição no número de inscrições na universidade.
...
&nbsp;&nbsp;&nbsp;&nbsp;No contexto do nosso projeto, utilizamos o *Value Proposition Canvas* como uma ferramenta estratégica para assegurar que a nossa aplicação *Web* atenda de maneira efetiva às necessidades dos usuários. Ao preencher este canvas, conseguimos identificar de forma clara as dores e os ganhos dos participantes, assim como as tarefas que eles precisam realizar. Isso nos possibilitou desenvolver o nosso projeto com um foco direcionado aos temas mais importantes para os nossos usuários.
...
5. **Código com pouca ou sem modularização** - Responsável: João.<br>Para mitigar esse risco, João implementará uma revisão contínua do código por meio de práticas de revisão de código entre os membros da equipe. Ele estabelecerá padrões claros de codificação e arquitetura de software para garantir a modularização adequada do código.
...
### User Story 02: Responder Questionários
...
<div align="center">
<sub>Figure 34 - High-Fidelity Prototype - Peer Evaluation locked Screen</sub>
<br>
<br>
  <img src="/documentos/assets/PeerAssesmentLock.png" alt="Peer Evaluation locked">
<br>
<br>
<sup>Source: Produced by the authors (2024)</sup>
</div>
...
<div align="center">
<sub>Figure 35 - High-Fidelity Prototype - Peer Assesment Questions Screen</sub>
<br>
<br>
  <img src="/documentos/assets/Questions.png" alt="Peer Assesment Questions Screen">
<br>
<br>
<sup>Source: Produced by the authors (2024)</sup>
</div>
...
<div align="center">
<sub>Figure 35 - High-Fidelity Prototype - Peer Assesment Result Screen</sub>
<br>
<br>
  <img src="/documentos/assets/PeerAssesment.png" alt="Peer Assesment Result Screen">
<br>
<br>
<sup>Source: Produced by the authors (2024)</sup>
</div>
...
&nbsp;&nbsp;&nbsp;&nbsp;Nesta seção, apresentamos o modelo relacional da nossa aplicação, construído na plataforma [SQL Toad](https://sql.toad.cz/). Este modelo é uma representação estruturada das entidades e de suas relações, fundamentais para o funcionamento e organização do sistema. As tabelas descritas nesta seção formam a espinha dorsal do nosso aplicativo, delineando como os dados são armazenados, relacionados e acessados.

<div align="center">
<sub>Figure 46 - Logical Model</sub>
<br>
<br>
  <img src="/documentos/assets/modelo_logico.png" alt="Modelo lógico">
<br>
<br>
<sup>Source: Produced by the authors (2024)</sup>
</div>
...
&nbsp;&nbsp;&nbsp;&nbsp;O modelo relacional delineia as interações entre as entidades essenciais do nosso sistema. As tabelas primárias, como `Users`, `Groups`, `Universes`, `Country`, `Forms`, `Questions` e `QuestionsTwo`, representam os elementos centrais da nossa aplicação.

&nbsp;&nbsp;&nbsp;&nbsp;Cada tabela foi cuidadosamente projetada para conter atributos relevantes e estabelecer conexões significativas entre si. Por exemplo, a tabela `Groups` guarda informações sobre os grupos do sistema, incluindo o nome do grupo e a referência ao universo ao qual pertence através do campo `id_universes`, que é uma chave estrangeira para a tabela `Universes`. Por outro lado, a tabela `Users` armazena dados dos usuários, como nome, país, biografia e referências a grupos através do campo `id_groups`, que conecta cada usuário a um grupo específico. Essa estrutura relacional garante uma organização eficiente e interligada dos dados no sistema.

#### Tabela `Users`
&nbsp;&nbsp;&nbsp;&nbsp;A tabela `Users` é responsável por armazenar informações sobre os usuários do sistema. Ela contém atributos como `name`, `country`, `bio`, `id_logins`, `email`, `password` e `id_groups`, que são essenciais para identificar e caracterizar os usuários. A chave estrangeira `id_groups` referencia a tabela `Groups`, associando cada usuário a um grupo específico. Essas informações são cruciais para a gestão de acesso e personalização de funcionalidades dentro do sistema.

#### Tabela `Groups`
&nbsp;&nbsp;&nbsp;&nbsp;A tabela `Groups` organiza os usuários em diferentes grupos. Ela possui os campos `id`, `name` e `id_universes`, onde `id_universes` é uma chave estrangeira que referencia a tabela `Universes`. Isso permite a categorização dos grupos dentro de universos específicos, facilitando a segmentação e organização dos dados.

#### Tabela `Universes`
&nbsp;&nbsp;&nbsp;&nbsp;A tabela `Universes` descreve os diferentes universos ou contextos no qual os grupos de usuários podem existir. Com o atributo `name`, esta tabela fornece uma estrutura para categorizar o ambiente ou domínio no qual os grupos operam. A relação com a tabela `Groups` através do `id_universes` assegura a correta atribuição de grupos aos universos correspondentes.

#### Tabela `Country`
&nbsp;&nbsp;&nbsp;&nbsp;A tabela `Country` descreve os diferentes países nos quais os usuários podem pertencer. Com os atributo `name`, esta tabela fornece uma estrutura para categorizar o lugar de origem dos usuários. A relação com a tabela `Country` através do `id_country` assegura a correta atribuição de país aos usuários.

#### Tabela `Questions`
&nbsp;&nbsp;&nbsp;&nbsp;A tabela `Questions` armazena a primeira parte das perguntas que podem ser usadas em formulários e avaliações. Ela contém os campos `id` e `question`, onde cada entrada representa uma questão única. Esta tabela é essencial para a criação de questionários e para a coleta de feedback, integrando-se com outras tabelas através de chaves estrangeiras quando necessário.

#### Tabela `QuestionsTwo`
&nbsp;&nbsp;&nbsp;&nbsp;A tabela `QuestionsTwo` armazena a segunda parte das perguntas que podem ser usadas em formulários e avaliações. Ela contém os campos `id` e `question`, onde cada entrada representa uma questão única. Esta tabela é essencial para a criação de questionários e para a coleta de feedback, integrando-se com outras tabelas através de chaves estrangeiras quando necessário.

#### Tabela `Forms`
&nbsp;&nbsp;&nbsp;&nbsp;A tabela `Forms` gerencia os formulários dentro do sistema. Com atributos como `assignment_date`, `title`, `rated_user`, `reviewer_user`, `feedback`, `id_users`, `id_questions` e `answer`, ela permite a criação e a manutenção de formulários de avaliação. As chaves estrangeiras `id_users` e `id_questions` garantem a associação correta dos formulários aos usuários e às perguntas relevantes, permitindo uma análise detalhada das respostas e feedbacks.


&nbsp;&nbsp;&nbsp;&nbsp;O modelo relacional apresentado fornece uma base sólida para o desenvolvimento e a compreensão da nossa aplicação. Ao implementar o modelo lógico em um esquema de banco de dados, garantimos a integridade e a consistência dos dados, além de facilitar consultas complexas e análises precisas.

&nbsp;&nbsp;&nbsp;&nbsp;Este modelo não apenas organiza as informações de forma eficiente, mas também promove a escalabilidade e a manutenibilidade do sistema. À medida que nossa aplicação evolui, o modelo relacional servirá como um guia para implementar novas funcionalidades e adaptar-se às demandas em constante mudança dos usuários.

&nbsp;&nbsp;&nbsp;&nbsp;Em suma, o modelo relacional é uma peça fundamental do nosso projeto, capacitando-nos a construir uma aplicação robusta, flexível e intuitiva para atender às necessidades dos nossos usuários.
...
</datatypes><table x="562" y="230" name="users">
...
<table x="464" y="463" name="questions">
<row name="id" null="1" autoincrement="1">
<datatype>INTEGER</datatype>
<default>NULL</default></row>
...
<datatype>VARCHAR</datatype>
<default>NULL</default></row>
<row name="rated-user" null="1" autoincrement="0">
<datatype>VARCHAR</datatype>
<default>NULL</default></row>
...
<table x="374" y="88" name="notifications">
<row name="id" null="1" autoincrement="1">
<datatype>INTEGER</datatype>
<default>NULL</default></row>
<row name="date" null="1" autoincrement="0">
...
<table x="255" y="410" name="universes">
<row name="id" null="1" autoincrement="1">
<datatype>INTEGER</datatype>
<default>NULL</default></row>
<row name="name" null="1" autoincrement="0">
...
<datatype>VARCHAR</datatype>
<default>NULL</default></row>
<row name="id_universes" null="1" autoincrement="0">
<datatype>INTEGER</datatype>
<default>NULL</default><relation table="universes" row="id" />
</row>
<key type="PRIMARY" name="">
<part>id</part>
</key>
...
&nbsp;&nbsp;&nbsp;&nbsp;As consultas SQL e a lógica proposicional são elementos fundamentais no desenvolvimento de aplicações web modernas. Consultas SQL (Structured Query Language) são utilizadas para interagir com bancos de dados, permitindo a recuperação, inserção, atualização e exclusão de dados de maneira eficiente. A lógica proposicional, por sua vez, é um ramo da lógica que estuda as relações entre proposições e condições, utilizando operadores lógicos como AND, OR e NOT. Ao combinar essas duas áreas, desenvolvedores podem criar consultas SQL complexas que filtram e manipulam dados de acordo com múltiplas condições específicas. Essa capacidade de realizar buscas refinadas e específicas é crucial para garantir que apenas os dados relevantes sejam recuperados, melhorando a performance e a eficácia das aplicações web. A compreensão e a aplicação correta da lógica proposicional nas consultas SQL são, portanto, habilidades essenciais para qualquer desenvolvedor que busca otimizar o desempenho e a funcionalidade de suas aplicações.<br>

&nbsp;&nbsp;&nbsp;&nbsp;Começando pela primeira tabela, o comando SQL redigido é utilizado para selecionar um registro específico da tabela **Grupo**, segundo as condições estabelecidas. Assim, Pensando em um gerenciamento de dados de determinados grupos, o banco de dados contém uma tabela na qual armazena informações dos grupos, como seu nome e o universo que pertence. Tais dados são muito relevantes para a organização e funcionamento da plataforma. Dessa forma, esse comando seria usado para validar que o grupo existe e pertence a um universo específico.<br>

#1 | ---
--- | ---
**Expressão SQL** | **SELECT** * **FROM** Group **WHERE** id = 'req.body.group' **AND** universe = 'req.body.universe';
**Proposições lógicas** | $A$: O id do grupo é 'req.body.group' (id = 'req.body.group') <br> $B$: O universo do grupo é 'req.body.universe' (universe = 'req.body.universe');
**Expressão lógica proposicional** | $A \land B$
**Tabela Verdade** | 

|  A  |  B  |  (A ∧ B)  |
|:---:|:---:|:---------:|
|  <font color="green">V</font>  |  <font color="green">V</font>  |  <font color="green">V</font>  |
|  <font color="green">V</font>  |  <font color="red">F</font>  |  <font color="red">F</font>  |
|  <font color="red">F</font>  |  <font color="green">V</font>  |  <font color="red">F</font>  |
|  <font color="red">F</font>  |  <font color="green">V</font>  |  <font color="red">F</font>  |

&nbsp;&nbsp;&nbsp;&nbsp; Já na segunda tabela, o comando desenvolvido insere determinadas informações na tabela nomeada **User**, que armazena informações de usuários da plataforma, como: email, senha e grupo. Tais dados são importantes para realizar o login de diferentes usuários corretamente, assim como, guardar qual é o grupo que aquele integrante está inserido.<br>

#2 | ---
--- | ---
**Expressão SQL** | **INSERT INTO** **User** (email, password, group) **VALUES** ('req.body.email', 'req.body.password', 'req.body.group');
**Proposições lógicas** | $A$: Um email é inserido com nome 'req.body.email' <br> $B$ uma senha é inserido com nome 'req.body.password' <br> $C$: Um grupo é inserido com nome 'req.body.group';
**Expressão lógica proposicional** | $A \land B \land C$ 
**Tabela Verdade** |

|  A  |  B  |  C  |  (A ∧ B ∧ C)  |
|:---:|:---:|:---:|:-------------:|
|  <font color="green">V</font>  |  <font color="green">V</font>  |  <font color="green">V</font>  |  <font color="green">V</font>  |
|  <font color="green">V</font>  |  <font color="green">V</font>  |  <font color="red">F</font>  |  <font color="green">V</font>  |
|  <font color="green">V</font>  |  <font color="red">F</font>  |  <font color="green">V</font>  |  <font color="red">F</font>  |
|  <font color="green">V</font>  |  <font color="red">F</font>  |  <font color="red">F</font>  |  <font color="red">F</font>  |
|  <font color="red">F</font>  |  <font color="green">V</font>  |  <font color="green">V</font>  |  <font color="red">F</font>  |
|  <font color="red">F</font>  |  <font color="green">V</font>  |  <font color="red">F</font>  |  <font color="red">F</font>  |
|  <font color="red">F</font>  |  <font color="red">F</font>  |  <font color="green">V</font>  |  <font color="red">F</font>  |
|  <font color="red">F</font>  |  <font color="red">F</font>  |  <font color="red">F</font>  |  <font color="red">F</font>  |

&nbsp;&nbsp;&nbsp;&nbsp; Partindo para a terceira tabela, analisando novamente a tabela **User**, nessa consulta é preciso deletar um usuário que foi cadastrado por engano e seus dados estão armazedos no banco de dados. Sendo assim, a tabela será consultada e buscará o nome e o grupo específico que esse usuário foi cadastrado para, aí sim, efetuar a função DELETE.<br>

#3 | ---
--- | ---
**Expressão SQL** | **DELETE FROM** User **WHERE** name = 'name_value' **AND** group = 'group_value';
**Proposições lógicas** | $A$: O registro contém nome 'name_value' <br> $B$: O registro está no grupo 'group_value', <br> $C$: O usuário é deletado;
**Expressão lógica proposicional** | $A \land B$
**Tabela Verdade** | 
...
&nbsp;&nbsp;&nbsp;&nbsp; Ao final, três tabelas de lógica proposicional foram desenvolvidas, sendo elas: INSERT, DELETE e SELECT juntamente a condição composta AND. Ademais, tais tabelas são de extrema relevância para o desenvolvimento do projeto e demostram uma funcional conexão entre diferentes partes do código juntamente ao banco de dados.<br>

## 3.6. WebAPI e endpoints 

Esta seção fornece a documentação da Web API da nossa aplicação, desenvolvida com o framework Sails.js. Esta seção é dedicada a fornecer uma visão abrangente dos diversos endpoints disponíveis, permitindo que você entenda como interagir com os recursos da aplicação de forma eficiente. Aqui, você encontrará detalhes sobre os endpoints utilizados para gerenciamento de usuários, grupos, países e universos, além de funcionalidades essenciais como autenticação e submissão de avaliações. Cada endpoint é descrito detalhadamente, incluindo o endereço, método HTTP (GET, POST, PUT, PATCH, DELETE), headers, corpo da requisição e formatos de resposta. Esta documentação foi criada para ser uma referência completa, garantindo que desenvolvedores possam integrar e utilizar nossa API de maneira eficaz e sem complicações.

### Rotas e Endpoints

As seguintes são as rotas e endpoints disponíveis na aplicação:
...
- **PATCH `/updateBio`**
  - **Descrição**: Atualiza a bio do usuário logado.
  - **Método**: PATCH
  - **Headers**: 
    - Content-Type: application/json
...
- **GET `/api/country`**
  - **Descrição**: Recupera uma lista de países.
  - **Método**: GET
  - **Headers**: 
    - Content-Type: application/json
  - **Body**: Não aplicável
...
- **GET `/api/group`**
  - **Descrição**: Recupera uma lista de grupos.
  - **Método**: GET
  - **Headers**: 
    - Content-Type: application/json
  - **Body**: Não aplicável
...
- **GET `/questions`**
  - **Descrição**: Renderiza a página de questões.
  - **Método**: GET
  - **Headers**: 
    - Content-Type: text/html
  - **Body**: Não aplicável
  - **Response**: HTML da página de questões

- **POST `/api/submit-evaluation`**
  - **Descrição**: Submete uma avaliação.
  - **Método**: POST
  - **Headers**: 
    - Content-Type: application/json
  - **Body**:
...
Esta seção forneceu uma documentação detalhada dos endpoints da Web API utilizados na aplicação. Ao longo deste seção, você encontrou informações essenciais sobre como interagir com cada endpoint, incluindo métodos HTTP, headers, bodies de requisição e formatos de resposta esperados. Esperamos que esta documentação sirva como um guia claro e útil para desenvolvedores, facilitando a integração e a utilização da API em diversos contextos de desenvolvimento. Com essas informações, você estará bem equipado para aproveitar todas as funcionalidades oferecidas pela nossa API, garantindo uma implementação bem-sucedida e eficiente.

# <a name="c4"></a>4. Desenvolvimento da Aplicação Web
O desenvolvimento da aplicação web envolve a criação de três versões distintas, cada uma projetada para atender diferentes necessidades e estágios do projeto. A primeira versão é o protótipo de baixa fidelidade, que se concentra em esboçar a estrutura básica e o fluxo de navegação da aplicação, permitindo ajustes rápidos e econômicos. A segunda versão é o protótipo de alta fidelidade, uma representação detalhada e interativa que simula o design final e as funcionalidades. A terceira versão é o produto final, que integra todas as funcionalidades e refinamentos visuais, pronto para o uso, mas ainda passível de refinamento.

## 4.1. Primeira versão da aplicação web 

## Relatório de Desenvolvimento da Primeira Versão da Aplicação Web

### Introdução

Nesta primeira fase do desenvolvimento da nossa aplicação web, focamos em criar os alicerces visuais e estruturais do projeto. Nosso objetivo principal foi transformar as ideias iniciais e conceitos em uma base tangível e navegável.

### Desenvolvimento dos Wireframes

Começamos elaborando wireframes detalhados para mapear a disposição dos elementos e a navegação do site. Esses wireframes serviram como o primeiro passo para visualizar a interface do usuário e garantir que a estrutura atendesse aos requisitos iniciais do projeto.

### Conversão dos Conceitos para HTML

A partir dos wireframes, avançamos para a implementação dos conceitos em HTML. Nesta etapa, criamos as primeiras páginas do site, posicionando botões e elementos conforme planejado. A estrutura HTML foi desenvolvida de forma a permitir futuras integrações e expansões, garantindo uma base sólida para o projeto.

### Implementação Inicial de CSS

Embora o foco principal tenha sido a estrutura HTML, começamos a aplicar estilos básicos em CSS. Esta implementação inicial de CSS foi mínima e visava principalmente garantir que os elementos tivessem uma aparência funcional e básica, preparando o terreno para futuras iterações de design.

### Ausência de Backend e Funcionalidades

Nesta primeira versão, ainda não implementamos nenhum backend ou funcionalidades interativas. O objetivo foi criar uma fundação visual e estrutural robusta, sobre a qual poderíamos construir funcionalidades avançadas nas próximas versões.

### Conclusão

A primeira versão da aplicação web estabeleceu a base visual e estrutural do projeto. Com os wireframes convertidos em páginas HTML e uma aplicação inicial de CSS, estamos bem posicionados para adicionar backend e funcionalidades nas próximas fases do desenvolvimento. Este alicerce nos permitirá evoluir a aplicação de maneira organizada e eficiente, garantindo que futuras iterações sejam construídas sobre uma base sólida.

### Página de Registro (Register)

<div align="center">
<sub>Figura - Página de Registro</sub>
<br>
<br>
<img src="/documentos/assets/Registerhtml.png" width="100%">
<br>
<br>
<sup>Fonte: Material produzido pelos autores (2024)</sup>
</div>

&nbsp;&nbsp;&nbsp;&nbsp;A página de registro é projetada para ser o primeiro passo na jornada do usuário dentro da aplicação. Ela serve para coletar informações essenciais dos usuários, como nome, e-mail e senha, e é fundamental para estabelecer a identidade do usuário dentro do sistema. O objetivo é fornecer um processo de registro seguro, eficiente e livre de estresse.

### Página de Login

<div align="center">
<sub>Figura - Página de Login</sub>
<br>
<br>
<img src="/documentos/assets/loginhtml.png" width="100%">
<br>
<br>
<sup>Fonte: Material produzido pelos autores (2024)</sup>
</div>

&nbsp;&nbsp;&nbsp;&nbsp;A página de login é onde os usuários validam suas credenciais para acessar suas contas. Ela é crucial para manter a segurança do usuário e da aplicação, garantindo que apenas indivíduos autorizados possam entrar.

### Página da Equipe (Team Page)

<div align="center">
<sub>Figura - Página da Equipe</sub>
<br>
<br>
<img src="/documentos/assets/teampagehtml.png" width="100%">
<br>
<br>
<sup>Fonte: Material produzido pelos autores (2024)</sup>
</div>

&nbsp;&nbsp;&nbsp;&nbsp;A página da equipe é um espaço dedicado à colaboração e ao networking. Ela exibe os membros da equipe com seus respectivos países, promovendo a diversidade e o espírito de equipe. O objetivo é facilitar a conexão entre os membros, permitindo que um clique em um nome leve ao perfil daquela pessoa, onde mais informações podem ser compartilhadas.

### Autoavaliação (Self Assessment)

<div align="center">
<sub>Figura - Página de Autoavaliação</sub>
<br>
<br>
<img src="/documentos/assets/selfevaluationhtml.png" width="100%">
<br>
<br>
<sup>Fonte: Material produzido pelos autores (2024)</sup>
</div>

&nbsp;&nbsp;&nbsp;&nbsp;A página de autoavaliação é uma ferramenta introspectiva que permite aos usuários avaliarem seu próprio desempenho. O objetivo é fornecer um espaço para reflexão pessoal e identificação de áreas de melhoria, incentivando o crescimento pessoal e profissional.

### Sobre Mim (About Me)

<div align="center">
<sub>Figura - Página Sobre Mim</sub>
<br>
<br>
<img src="/documentos/assets/Aboumehtml.png" width="100%">
<br>
<br>
<sup>Fonte: Material produzido pelos autores (2024)</sup>
</div>

&nbsp;&nbsp;&nbsp;&nbsp;A página 'Sobre Mim' é o perfil pessoal do usuário, onde ele pode compartilhar sua biografia, nome e país. O objetivo é humanizar a experiência online, permitindo que os usuários expressem sua identidade e se conectem em um nível mais pessoal com outros membros da equipe.

O desenvolvimento do back-end representou o desafio mais significativo do nosso projeto, uma verdadeira prova das nossas habilidades técnicas e capacidade de resolução de problemas. As complexidades inerentes à construção de uma infraestrutura de back-end robusta e escalável exigiram um estudo aprofundado e uma dedicação sem precedentes. Foi necessário mergulhar profundamente em conceitos avançados de programação, estruturas de dados e algoritmos. A assistência dos professores foi inestimável, proporcionando orientação e insights que nos permitiram superar os obstáculos mais desafiadores e garantir que nossa aplicação fosse construída sobre uma fundação sólida.

;Por outro lado, o front-end demandou uma abordagem meticulosa, focada na criação de uma interface de usuário que não só fosse visualmente atraente, mas também intuitiva e responsiva. O desafio aqui foi equilibrar a estética com a funcionalidade, garantindo que cada elemento de design servisse a um propósito e melhorasse a experiência geral do usuário. Embora as questões de front-end tenham sido menos complexas do que as do back-end, elas exigiram uma atenção igualmente rigorosa aos detalhes, desde a tipografia e paleta de cores até a acessibilidade e a interatividade dos elementos na tela.

Para aprimorar nossa aplicação, temos uma visão estratégica que abrange várias áreas de desenvolvimento, todas com o objetivo de elevar a qualidade e a eficiência da nossa plataforma. Entendemos que o processo de melhoria é contínuo e requer um compromisso com a inovação e a excelência operacional. Com isso em mente, delineamos os seguintes planos:

Nosso primeiro passo é refinar a interface do usuário para garantir que cada elemento visual não apenas encante, mas também facilite a navegação do usuário. Isso significa alinhar cada página com os protótipos detalhados criados no Figma, onde cada cor, ícone e animação foi cuidadosamente escolhido para criar uma experiência coesa e intuitiva. Vamos revisitar cada pixel, cada transição e cada interação para garantir que a realidade da nossa aplicação reflita a visão original dos nossos designers.

No que diz respeito ao tratamento de dados, nosso objetivo é implementar procedimentos que garantam a consistência e a precisão dos dados antes de serem enviados para o banco de dados. Isso envolve a validação e a formatação dos dados no lado do cliente, assegurando que todas as informações estejam padronizadas e livres de erros antes de serem armazenadas. Com essas medidas, pretendemos evitar a inserção de dados despadronizados ou incorretos no banco de dados, o que é crucial para manter a integridade e a confiabilidade do sistema.

A fluidez da experiência do usuário depende de uma integração entre o front-end e o back-end. Estamos comprometidos em reforçar essa integração, utilizando as melhores práticas e as tecnologias mais recentes para garantir que a comunicação entre as diferentes partes da aplicação seja rápida e sem erros. Isso inclui a melhoria da lógica de negócios no servidor e a garantia de que os processos de back-end suportem efetivamente as funcionalidades de front-end. Com essas melhorias, esperamos não apenas aumentar a velocidade e a confiabilidade da aplicação, mas também facilitar a implementação de novas funcionalidades no futuro.

Esses passos são fundamentais para o crescimento e a evolução da nossa aplicação. Com eles, buscamos não apenas atender, mas superar as expectativas dos nossos usuários, fornecendo uma plataforma que seja poderosa, segura e agradável de usar.

Este relatório de desenvolvimento reflete a jornada da nossa equipe até o momento, marcada por conquistas significativas e aprendizados valiosos. As páginas que desenvolvemos são o resultado de um trabalho em equipe focado e uma visão compartilhada de excelência. As dificuldades que superamos nos fortaleceram e os próximos passos que planejamos nos motivam a seguir em frente, sempre com o objetivo de entregar uma aplicação web que seja não apenas uma ferramenta, mas uma extensão da identidade e das aspirações dos nossos usuários.

### Fechamento

Como pode ser observado, a primeira versão focou em estabelecer uma base robusta. Algumas páginas e funcionalidades ainda estão ausentes, mas este é um passo estratégico para garantir um desenvolvimento ágil e eficaz. Na próxima versão, pretendemos começar a implementar o backend e adicionar funcionalidades interativas. Continuaremos ajustando e melhorando o escopo conforme necessário para atender aos objetivos do projeto.


## 4.2. Segunda versão da aplicação web 

&nbsp;&nbsp;&nbsp;&nbsp;Na Segunda versão da nossa aplicação web, focamos em refinar e ajustar elementos cruciais para melhorar a experiência do usuário e a eficiência do desenvolvimento. A seguir, detalhamos as principais mudanças e melhorias realizadas nesta fase:

### Ajustes de CSS e Layout

Desde a primeira versão, nosso CSS estava bem desenvolvido, proporcionando uma base sólida para a interface do usuário. Nesta fase, realizamos ajustes finos em algumas páginas para garantir uma aparência e funcionalidade consistentes. As modificações incluíram:

- **Correção de Alinhamentos**: Pequenos ajustes no alinhamento dos elementos para melhorar a estética e usabilidade.
- **Melhorias em Responsividade**: Ajustes adicionais para garantir que a aplicação funcione perfeitamente em dispositivos móveis e diferentes tamanhos de tela.
- **Aprimoramento Visual**: Aplicação de melhorias visuais sutis, como sombras e transições suaves, para uma experiência de usuário mais agradável.

### Alterações no Escopo e Backend

Uma das mudanças mais significativas nesta versão foi a revisão do escopo do projeto e a reestruturação do backend. Devido à limitação de tempo e recursos, decidimos simplificar nossa aplicação removendo algumas páginas e funcionalidades que, embora planejadas inicialmente, não eram essenciais para o MVP (Minimum Viable Product). Entre as principais alterações, destacamos:

- **Remoção da Página de Relatórios**: A página de relatórios foi retirada do escopo nesta versão. Embora fosse uma funcionalidade valiosa, a sua complexidade exigia mais tempo de desenvolvimento do que podíamos alocar no momento.
- **Otimização do Backend**: Com a redução no escopo, aproveitamos para otimizar o backend, removendo código desnecessário e melhorando a eficiência das funcionalidades restantes. Isso incluiu:
  - **Refatoração de Código**: Simplificação e limpeza do código para melhorar a manutenção e a performance.
  - **Ajustes em APIs**: Revisão das APIs para garantir que todas as chamadas de backend fossem rápidas e eficientes, eliminando endpoints redundantes.


### Create Universe and Group Page

<div align="center">
<sub>Figura - Create Universe and Group Page</sub>
<br>
<br>
<img src="/documentos/assets/UniversePage.png" width="100%">
<br>
<br>
<sup>Fonte: Material produzido pelos autores (2024)</sup>
</div>
...
<div align="center">
<sub>Figura - Create User Page</sub>
<br>
<br>
<img src="/documentos/assets/RegisterCSS.png" width="100%">
<br>
<br>
<sup>Fonte: Material produzido pelos autores (2024)</sup>
</div>
...
<div align="center">
<sub>Figura - Login Page</sub>
<br>
<br>
<img src="/documentos/assets/LoginCSS.png" width="100%">
<br>
<br>
<sup>Fonte: Material produzido pelos autores (2024)</sup>
</div>
...
<div align="center">
<sub>Figura - Team Page</sub>
<br>
<br>
<img src="/documentos/assets/TeamPageCSS.png" width="100%">
<br>
<br>
<sup>Fonte: Material produzido pelos autores (2024)</sup>
</div>
...
<div align="center">
<sub>Figura - About Others Page</sub>
<br>
<br>
<img src="/documentos/assets/AboutOthersCSS.jpg" width="100%">
<br>
<br>
<sup>Fonte: Material produzido pelos autores (2024)</sup>
</div>
...
<div align="center">
<sub>Figura - About Me Page</sub>
<br>
<br>
<img src="/documentos/assets/AboutMeCSS.jpg" width="100%">
<br>
<br>
<sup>Fonte: Material produzido pelos autores (2024)</sup>
</div>

### Self Assessment PAge Locked

<div align="center">
<sub>Figura - Self Assessment Page Locked</sub>
<br>
<br>
<img src="/documentos/assets/Self-Assessment-Page-Locked.png" width="100%">
<br>
<br>
<sup>Fonte: Material produzido pelos autores (2024)</sup>
</div>

### Questions for Self Assesment PAge

<div align="center">
<sub>Figura - Questions for Self Assesment PAge</sub>
<br>
<br>
<img src="/documentos/assets/QuestionsCSS.jpg" width="100%">
<br>
<br>
<sup>Fonte: Material produzido pelos autores (2024)</sup>
</div>

### Self Assesment Page Results

<div align="center">
<sub>Figura - Self Assesment Page Results</sub>
<br>
<br>
<img src="/documentos/assets/SelfAssesmentCSS.png" width="100%">
<br>
<br>
<sup>Fonte: Material produzido pelos autores (2024)</sup>
</div>
...
### Peer Assessment Page Results

<div align="center">
<sub>Figura - Peer Assessment Page Results</sub>
<br>
<br>
<img src="/documentos/assets/PeerAssesmentCSS.png" width="100%">
<br>
<br>
<sup>Fonte: Material produzido pelos autores (2024)</sup>
</div>
...
As decisões tomadas nesta fase foram críticas para manter o projeto dentro do prazo e garantir que entregássemos uma aplicação funcional e de qualidade. A simplificação do escopo permitiu-nos focar em funcionalidades essenciais, assegurando uma base sólida para futuras iterações e melhorias


## 4.3. Versão final da aplicação 

## Relatório de Desenvolvimento da Terceira Versão da Aplicação Web (Versão Final)

Na Terceira e última versão da nossa aplicação web, focamos em finalizar as funcionalidades e ajustes necessários para garantir uma entrega de alta qualidade. A seguir, detalhamos as principais mudanças e melhorias realizadas nesta fase:

### Ajustes de CSS e Layout

Nesta etapa final, realizamos modificações estéticas e funcionais em diversas páginas para aprimorar a experiência do usuário e a usabilidade da aplicação. Entre as principais mudanças, destacamos:

**Redução do Escopo**: Considerando o tempo restante para a entrega final, reduzimos o escopo do projeto. Removemos algumas páginas do nosso backlog para concentrar nossos esforços nas funcionalidades essenciais.

**Alterações na Team Page**: A maior diferença estética foi observada na página da equipe (Team Page). Realizamos mudanças significativas no design para adaptá-la ao funcionamento desejado, tornando-a mais intuitiva e visualmente agradável.

### Alterações no Escopo e Backend

Com a necessidade de cumprir o prazo final, fizemos uma revisão cuidadosa do escopo e implementamos ajustes no backend para garantir que todas as funcionalidades críticas estivessem operacionais. As principais alterações incluem:

**Remoção de Páginas do Backlog**: Identificamos e removemos páginas menos prioritárias para focar em aspectos fundamentais da aplicação.
**Otimização do Backend**: Continuamos a otimização do backend para garantir eficiência e desempenho. Isso envolveu:

  - **Refatoração de Código**: Mais uma rodada de simplificação e limpeza de código para garantir a manutenibilidade e a performance.
  - **Finalização de Funcionalidades**: Implementação completa das funcionalidades restantes nas páginas principais, assegurando uma experiência de usuário coesa e sem falhas.


### Dificuldades

O grupo apresentou dificuldades quanto ao funcionamento do backend. Boa parte dos membros do grupo não sabia como criar um controller, como criar um model e como conectar o backend com o frontend.

Além disso, o grupo como um todo teve dificuldade em seguir o Gitflow. Tivemos diversos problemas com branches ao longo do processo, incluindo problemas com merges e commits que sobrescreviam o trabalho de outros membros.

Outra dificuldade significativa foi na organização das tarefas e do escopo do projeto. Desenvolvemos diversas tarefas e páginas que não foram para frente, resultando em desperdício de tempo e esforço

### Impacto das Mudanças

As decisões tomadas nesta fase final foram cruciais para garantir a entrega de uma aplicação funcional e bem-acabada. A redução estratégica do escopo permitiu-nos focar em funcionalidades essenciais e garantir que todas fossem implementadas com a qualidade necessária.

### Sucessos

- Dentro do grupo, tivemos vários sucessos  durante o desenvolvimento da aplicação web. Esses sucessos incluem a entrega do MVP, a integração entre páginas e a finalização de várias funcionalidades importantes. A seguir, detalhamos cada um desses sucessos:

**Entrega do MVP**

Conseguimos entregar um produto mínimo viável (MVP) que atende às funcionalidades básicas pedidas. Esse sucesso é significativo, demonstra nossa capacidade de cumprir os requisitos essenciais do projeto dentro do prazo estabelecido.

**Integração entre Páginas**

A integração das páginas foi um sucesso. Conseguimos conectar diferentes partes da aplicação, garantindo uma navegação fluida e coerente para o usuário. Isso inclui a ligação entre a página inicial, as páginas de login e registro, e outras seções importantes do site.

**Página AboutMe**

A página AboutMe, proposta no início do projeto, foi concluída com êxito. Esta página fornece informações detalhadas sobre o usuário, contribuindo para uma experiência personalizada e enriquecedora dentro da aplicação.

**Página TeamPage**

A TeamPage, que foi proposta durante o desenvolvimento, também foi entregue e está totalmente funcional. Esta página permite que os usuários vejam informações sobre os membros da equipe, promovendo a transparência e o espírito de colaboração.

**Página Rating**

A página Rating, que contém as perguntas para validar o perfil dos usuários, foi um dos principais critérios que queríamos introduzir na aplicação. Conseguimos implementar esta funcionalidade com sucesso. A página permite a coleta de feedback e autoavaliações, fornecendo informações valiosas para os usuários e para a melhoria contínua da aplicação.
Cada um desses sucessos reflete nosso compromisso com a qualidade e a funcionalidade da aplicação, bem como nossa capacidade de trabalhar de forma colaborativa e eficiente.

### Próximos Passos

Esta versão final marca o término de um ciclo importante de desenvolvimento. Embora esta seja a entrega final para este projeto, reconhecemos que melhorias contínuas são sempre possíveis. Futuros aprimoramentos poderão incluir a reavaliação de funcionalidades removidas e a incorporação de novos feedbacks dos usuários para continuar evoluindo a aplicação.
...
<br>
<br>
<img src="/documentos/assets/newTeamPage.jpeg" width="100%">
<br>
<br>
<sup>Fonte: Material produzido pelos autores (2024)</sup>
</div>
...
Como pode ser observado de forma visual, algumas páginas foram retiradas nesta versão para simplificar e focar em funcionalidades essenciais. Para esta versão final, terminamos de implementar o backend e as funcionalidades das páginas. Caso necessário, ajustamos novamente o escopo para garantir que estamos entregando uma aplicação de alta qualidade dentro do prazo.

Estamos comprometidos com a melhoria contínua da nossa aplicação e confiantes de que esta versão final oferece uma base sólida e funcional para os nossos usuários.


# <a name="c5"></a>5. Testes

Neste tópico, apresentamos os resultados dos testes de usabilidade realizados com usuários reais e dos testes automatizados aplicados à nossa aplicação web. Os testes de usabilidade foram conduzidos para avaliar a experiência do usuário, identificando pontos fortes e áreas que precisam de melhorias. Usuários reais interagiram com a aplicação, e suas opiniões e dificuldades foram registradas para nos ajudar a tornar o sistema mais intuitivo e fácil de usar. Paralelamente, realizamos testes automatizados para verificar a funcionalidade, desempenho e segurança da aplicação. Esses testes foram programados para executar uma série de ações repetitivas e complexas, garantindo que a aplicação funcione corretamente sob diferentes condições. Combinando os insights dos testes de usabilidade com a precisão dos testes automatizados, conseguimos uma visão abrangente da qualidade e eficiência da nossa aplicação web.

## 5.1. Relatório de testes automatizados 

Este documento descreve os testes automatizados planejados e implementados para a nossa solução. Os testes cobrem diversos aspectos dos controladores da aplicação, assegurando que os métodos funcionam conforme esperado. Abaixo, listamos e descrevemos cada teste realizado, indicando o que eles fazem e os resultados obtidos.

### 5.1.1. Testes do UniverseController

#### Teste: create()

##### Descrição:
Este teste verifica a funcionalidade do método `create` no `UniverseController`. Ele certifica que um novo universo pode ser criado corretamente quando um nome válido é fornecido e que um erro é retornado se o nome não for fornecido.
...
- Teste de criação de universo com nome válido: Passou.
- Teste de erro de badRequest quando o nome não é fornecido: Passou.

**5.1.2. Testes do UserController**

**Teste**: deve retornar uma lista de usuários
**Descrição:**

Este teste verifica se o método que retorna a lista de usuários funciona corretamente, populando corretamente os grupos de usuários.
...
Teste de retorno da lista de usuários: Passou.




**Teste**: deve retornar erro de servidor em caso de falha
**Descrição:**

Este teste verifica se o método retorna um erro de servidor (status 500) quando ocorre uma falha no banco de dados.
...
- Teste de erro de servidor em caso de falha no banco de dados: Passou.




**Teste**: deve hashear a senha antes de criar o usuário
**Descrição:**

Este teste verifica se a senha do usuário é hasheada corretamente antes de criar o usuário no banco de dados.
...
Teste de hashing de senha: Passou.




**Teste:** deve chamar proceed com erro se hash falhar
**Descrição:**

Este teste verifica se o método chama proceed com um erro se o hashing da senha falhar.
...
**Resultados:**
 ...
 - Teste de erro no hashing de senha: Passou.




 **Teste:** deve criar um novo usuário com sucesso
**Descrição:**

Este teste verifica se um novo usuário é criado corretamente quando os dados válidos são fornecidos.
...
describe('UserController create', () => {
  it('deve criar um novo usuário com sucesso', async () => {
    ...
    const createUserStub = sinon.stub(User, 'create').resolves({ ...userData, id: 1 });
    ...
    assert.deepStrictEqual(response.body, { ...userData, id: 1 });
    assert(createUserStub.calledOnceWith(userData));
  });
});
```
...
Teste de criação de novo usuário: Falhou.

**Erro:**

<img src="/documentos/assets/Erro3TesteUser.PNG" width="100%">

O valor retornado não é estritamente igual ao esperado.




**Teste:** deve retornar erro 400 se os dados estiverem faltando
**Descrição:**


Este teste verifica se o método retorna um erro 400 (Bad Request) quando os dados obrigatórios estão faltando na requisição.
...
Teste de erro 400 para dados faltando: Passou.





**Teste:** deve retornar um usuário existente ao fazer login com sucesso
**Descrição:**

Este teste verifica se um usuário existente é retornado corretamente quando as credenciais de login são válidas.
...
Teste de login com sucesso: Falhou.

**Erro:**

<img src="/documentos/assets/Erro2TesteUser.PNG" width="100%">
...
**Teste:** deve retornar erro 400 se o e-mail ou senha estiverem incorretos
**Descrição:**


Este teste verifica se o método retorna um erro 400 (Bad Request) quando o e-mail ou senha fornecidos estão incorretos.
...
Teste de erro 400 para login com credenciais incorretas: Falhou.
...
&nbsp;&nbsp;&nbsp;&nbsp;Um teste de usabilidade é um método de avaliação de um produto no qual usuários reais interagem com ele para identificar problemas e oportunidades de melhoria. Durante o teste, observa-se como os usuários executam tarefas específicas no produto, registrando dificuldades, confusões e comportamentos inesperados. A principal vantagem dos testes de usabilidade é proporcionar uma compreensão profunda do comportamento dos usuários em situações reais de uso, permitindo avaliar se eles conseguem entender e utilizar as funcionalidades do produto de maneira eficiente. Esse processo fornece informações essenciais que ajudam os desenvolvedores a corrigir problemas de usabilidade e a aprimorar a experiência do usuário.

### 5.2.1 Registros de testes e melhorias

Relatórios (tabelas) de testes de usabilidade:
(https://docs.google.com/spreadsheets/d/1-dP8v9JHvfEy0bwbRkOz1RMrorA7leWwZPDmJglRx_w/edit?usp=sharing)

&nbsp;&nbsp;&nbsp;&nbsp;Nosso teste de usabilidade foi realizado no dia 13 de junho, durante o período da manhã, começando às 11h e terminando às 12h. O grupo testou o produto com seis participantes. O primeiro a testar a nossa aplicação foi o professor Victor Hayashi, especialista em pensamento computacional. Após ele, os demais testadores eram alunos do primeiro ano do Inteli, sendo eles: Kesney Lucas, Pedro Pinheiro, Enzo Salvador, Danilo Martins e Lucas Magalhães.

&nbsp;&nbsp;&nbsp;&nbsp;O primeiro problema identificado em nosso teste foi um erro de comunicação dentro do grupo. Assumimos que tanto o computador do Pedro quanto o do David estavam completamente atualizados, mas apenas o computador do Pedro estava atualizado. Essa discrepância resultou em experiências de usuário distintas para aqueles que testaram em cada uma dessas máquinas, gerando resultados variados. Os usuários que testaram no computador do Pedro foram Victor, Enzo e Lucas, enquanto Kesney, Pedro e Danilo testaram no computador do David.

### Tarefa 1 - Fazer Cadastro

&nbsp;&nbsp;&nbsp;&nbsp;Essa tarefa contempla a criação de um usuário, dividida em 7 etapas, sendo elas: inserir o nome, inserir o e-mail, inserir a senha, confirmar a senha, selecionar o grupo, selecionar o país e criar o usuário. O resultado esperado é a criação do usuário, necessária para a tarefa seguinte. Para representar o teste, criamos um gráfico de pizza, para representar a porcentagem de pessoas que tiveram sucesso, que conseguiram mas com dificuldade e que não conseguiram realizar a tarefa.

<div align="center">
<sub>Figure - Gráfico Tarefa 1</sub>
<br>
<br>
<img src="/documentos/assets/fazerCadastro.png" width="100%">
<br>
<br>
<sup>Source: Produced by the authors (2024)</sup>
</div>
...
&nbsp;&nbsp;&nbsp;&nbsp;Com base na análise do gráfico, conclui-se que todos os usuários que testaram a aplicação conseguiram executar a tarefa com sucesso e não relataram dificuldades significativas. Assim, não identificamos nenhum ponto crítico de melhoria que precise ser abordado para a entrega do projeto.

### Tarefa 2 - Fazer Login

&nbsp;&nbsp;&nbsp;&nbsp;Essa tarefa abrange o processo de login em nossa página, utilizando o usuário criado na etapa anterior. Dividimos a tarefa em três etapas: inserir o nome de usuário, inserir a senha e realizar o login. O resultado esperado é que o usuário consiga fazer login na página, permitindo acesso às demais seções de nossa aplicação. Para ilustrar os resultados do teste, criamos um gráfico de pizza, representando a porcentagem de usuários que obtiveram sucesso, aqueles que conseguiram com dificuldades, e os que não conseguiram concluir a tarefa.

<div align="center">
<sub>Figure - Gráfico Tarefa 2</sub>
<br>
<br>
<img src="/documentos/assets/fazerLogin.png" width="100%">
<br>
<br>
<sup>Source: Produced by the authors (2024)</sup>
</div>
...
&nbsp;&nbsp;&nbsp;&nbsp;Com base na análise do gráfico de pizza, constatamos que todos os usuários que testaram a aplicação conseguiram executar a tarefa de login com sucesso, sem encontrar dificuldades significativas. Esse resultado indica que o processo de login está bem implementado e intuitivo. Portanto, não identificamos nenhum ponto crítico de melhoria para essa funcionalidade específica na entrega do projeto.

### Tarefa 3 - Responder Formulário

&nbsp;&nbsp;&nbsp;&nbsp;Essa tarefa contempla a execução da autoavaliação da nossa aplicação e foi dividida em 4 etapas, sendo elas: entrar na página de questionário, selecionar as respostas e submeter as respostas. O resultado esperado é a conclusão do questionário, no qual o usuário verá o resultado da sua autoavaliação. Para representar o teste, criamos um gráfico de pizza, para representar a porcentagem de pessoas que tiveram sucesso, que conseguiram mas com dificuldade e que não conseguiram realizar a tarefa.

<div align="center">
<sub>Figure - Gráfico Tarefa 3</sub>
<br>
<br>
<img src="/documentos/assets/responderFormulario.png" width="100%">
<br>
<br>
<sup>Source: Produced by the authors (2024)</sup>
</div>
...
&nbsp;&nbsp;&nbsp;&nbsp;A análise do gráfico revelou que nossa página de questionários possui problemas significativos. Dos seis usuários, três não conseguiram completar a tarefa ao utilizar o computador do David, devido ao nível de zoom da página que impedia a visualização do botão de envio submit. Esse problema de visualização resultou na não conclusão do questionário por esses usuários. Em contrapartida, os outros três usuários que testaram a tarefa no computador do Pedro não enfrentaram problemas relacionados ao zoom e conseguiram localizar o botão de envio. No entanto, mesmo após a conclusão do questionário, o sistema não retornou o resultado ao usuário, levando a uma conclusão da tarefa com dificuldades.

&nbsp;&nbsp;&nbsp;&nbsp;O primeiro problema foi causado pelo uso de uma máquina desatualizada para o teste, algo que já foi resolvido na versão mais recente do sistema. O segundo problema, entretanto, é considerado crítico, pois é essencial que os usuários tenham acesso ao resultado de sua autoavaliação. Portanto, essa falha deve ser corrigida com urgência para garantir uma experiência satisfatória.

### Tarefa 4 - Criar Universo e Grupo

&nbsp;&nbsp;&nbsp;&nbsp;Essa tarefa envolve a criação de universo e grupo dentro do projeto, uma página especificamente utilizada pelo tutor. O processo foi dividido em cinco etapas: preencher o nome do universo, criar o universo, preencher o nome do grupo, atribuir o grupo ao universo e, finalmente, criar o grupo. O resultado esperado é que o universo e o grupo sejam criados com sucesso e que o grupo seja corretamente associado ao universo. Para ilustrar os resultados do teste, elaboramos um gráfico de pizza que representa a porcentagem de usuários que obtiveram sucesso, aqueles que conseguiram realizar a tarefa com dificuldades e os que não conseguiram concluí-la.

<div align="center">
<sub>Figure - Gráfico Tarefa 4</sub>
<br>
<br>
<img src="/documentos/assets/criarUniversoEGrupo.png" width="100%">
<br>
<br>
<sup>Source: Produced by the authors (2024)</sup>
</div>
...
&nbsp;&nbsp;&nbsp;&nbsp;A análise do gráfico revelou problemas na nossa página de criação de universos e grupos destinada aos tutores. Três dos participantes não conseguiram concluir a tarefa, especificamente na quarta etapa, onde, mesmo após preencher todos os campos, ao pressionar o botão de criar, o sistema retornava um erro, impedindo a criação do grupo. Esses três usuários estavam utilizando o computador do David, que não estava atualizado. Em contraste, os outros três participantes que utilizaram o computador do Pedro, que estava atualizado, conseguiram realizar a tarefa com sucesso.

&nbsp;&nbsp;&nbsp;&nbsp;Com base nesses resultados, concluímos que os erros observados durante os testes foram causados pela utilização de uma versão desatualizada da máquina do David. Portanto, não há necessidade de correções adicionais, uma vez que o problema já foi solucionado na versão mais recente do sistema.

### Tarefa 5 - Navegar pelo Site

&nbsp;&nbsp;&nbsp;&nbsp;Essa tarefa avalia a navegação pelo site, focando em como os usuários se movem dentro da nossa aplicação. Dividimos a tarefa em três etapas: acessar a página da equipe Team Page, acessar a página de avaliação Rating Page e acessar a página de perfil Profile Page. O objetivo deste teste é compreender como os usuários navegam no site, permitindo-nos identificar tanto problemas quanto pontos fortes da nossa aplicação. Para ilustrar os resultados, criamos um gráfico de pizza que representa a porcentagem de usuários que obtiveram sucesso, aqueles que completaram a tarefa com dificuldades e os que não conseguiram concluí-la.

<div align="center">
<sub>Figure - Gráfico Tarefa 5</sub>
<br>
<br>
<img src="/documentos/assets/navegarPeloSite.jpeg" width="100%">
<br>
<br>
<sup>Source: Produced by the authors (2024)</sup>
</div>
...
&nbsp;&nbsp;&nbsp;&nbsp;A análise do gráfico revelou problemas significativos na navegação entre as telas da nossa aplicação. Embora todos os testadores tenham conseguido executar a tarefa de navegação, o site apresentou erros em ambas as máquinas utilizadas para o teste.

&nbsp;&nbsp;&nbsp;&nbsp;No computador do David, o problema ocorria ao clicar na opção Team no cabeçalho enquanto o usuário já estava na página da equipe Team Page, resultando em um erro. No computador do Pedro, ao realizar a mesma ação, o usuário era redirecionado para a página de login, em vez de permanecer na Team Page.

&nbsp;&nbsp;&nbsp;&nbsp;É crucial corrigir esses problemas de navegação, pois, embora sejam simples de resolver, eles impactam significativamente a experiência do usuário. Garantir uma navegação fluida e sem erros é fundamental para a usabilidade da aplicação.

### Tarefa 6 - Editar Perfil

&nbsp;&nbsp;&nbsp;&nbsp;Essa tarefa envolve a edição do perfil do usuário e foi dividida em quatro etapas: editar a biografia, editar o nome, editar o país e editar a faculdade. O objetivo deste teste é garantir que o perfil do usuário seja atualizado corretamente com as informações fornecidas em cada uma dessas etapas, refletindo as características pessoais do usuário.

<div align="center">
<sub>Figure - Gráfico Tarefa 6</sub>
<br>
<br>
<img src="/documentos/assets/editarPerfil.png" width="100%">
<br>
<br>
<sup>Source: Produced by the authors (2024)</sup>
</div>
...
&nbsp;&nbsp;&nbsp;&nbsp;A análise do gráfico revelou que há problemas na parte de edição do perfil do usuário. Dos três testes realizados no computador do David, apenas o primeiro usuário conseguiu concluir a tarefa com sucesso. Isso ocorreu porque as alterações feitas no perfil não estavam sendo reiniciadas entre os testes, o que obrigava o segundo usuário a apagar as informações previamente preenchidas antes de editar seu próprio perfil.

&nbsp;&nbsp;&nbsp;&nbsp;Nos testes realizados no computador do Pedro, o problema foi diferente: os usuários não completaram a tarefa integralmente, editando apenas alguns dos campos que compõem o perfil, como biografia, nome, país e faculdade.

&nbsp;&nbsp;&nbsp;&nbsp;O primeiro problema precisa ser resolvido com urgência, pois os usuários não devem encontrar informações pré-preenchidas por outros usuários ao editar seus perfis. Isso compromete a privacidade e a individualidade das configurações do perfil. O segundo problema, embora menos crítico, deve ser abordado para garantir que os usuários preencham todos os campos do perfil. O grupo deve considerar implementar uma abordagem que incentive ou obrigue o preenchimento completo dos campos do perfil para uma melhor experiência do usuário.

### Itens que necessitam conserto

- *Tarefa 3*: Permitir que o usuário visualize o resultado do questionário.
- *Tarefa 5: Fazer com que o botão *Team não possua nenhuma funcionalidade quando o usuário já está na Team Page.
- *Tarefa 6*: Corrigir para que os dados do perfil de um usuário não apareçam em outro perfil.
- *Tarefa 6*: Criar um mecanismo que impeça o usuário de deixar de editar todos os itens do seu perfil.

### O que foi feito

- *Tarefa 3*: O erro foi consertado. Agora, ao finalizar a autoavaliação, o usuário pode visualizar o resultado do questionário.
- *Tarefa 5: O erro foi consertado. Agora, o botão *Team na Team Page não possui nenhuma função quando o usuário já está nessa página.
- *Tarefa 6*: O primeiro erro foi consertado. Agora, os dados de um usuário não são mais exibidos em outros perfis.
- *Tarefa 6*: O segundo erro não foi consertado. No entanto, o usuário agora só precisa editar a biografia, pois os outros dados são importados automaticamente do cadastro.

### Relatório de Teste SUS

&nbsp;&nbsp;&nbsp;&nbsp;O Sistema de Avaliação de Usabilidade (SUS) é uma ferramenta simples e eficaz para medir a usabilidade de produtos. O SUS utiliza um questionário composto por dez perguntas que os usuários respondem após utilizar o produto. Cada pergunta é classificada em uma escala de 1 a 5, e as respostas são convertidas em uma pontuação que varia de 0 a 100, indicando a facilidade de uso percebida. A simplicidade do SUS permite sua aplicação em uma ampla variedade de contextos, facilitando para os desenvolvedores a identificação de pontos de melhoria na experiência do usuário de maneira eficiente.

&nbsp;&nbsp;&nbsp;&nbsp;Nosso teste foi realizado com a participação de seis pessoas. No entanto, três dessas pessoas utilizaram uma máquina que não estava com a versão mais atualizada do sistema. Por esse motivo, calculamos a média das pontuações de todos os seis testadores, bem como a média das pontuações dos testadores em cada máquina, para avaliar de forma mais precisa o impacto das diferentes versões na usabilidade percebida.

&nbsp;&nbsp;&nbsp;&nbsp;As pessoas que testaram na máquina desatualizada foram Kesney Lucas, Pedro Pinheiro e Danilo Martins. Abaixo está o gráfico de barras que representa o resultado do Teste SUS para esses participantes.

<div align="center">
<sub>Figure - Teste Sus - Máquina Desatualizada</sub>
<br>
<br>
<img src="/documentos/assets/testeSusDesatualizado.png" width="100%">
<br>
<br>
<sup>Source: Produced by the authors (2024)</sup>
</div>
...
&nbsp;&nbsp;&nbsp;&nbsp;O grupo que testou a versão mais atualizada do projeto foi composto por Victor Hayashi, Enzo Salvador e Lucas Magalhães. Abaixo está o gráfico de barras que representa os resultados do Teste SUS para esses participantes.

<div align="center">
<sub>Figure 02 - Teste Sus - Máquina Atualizada</sub>
<br>
<br>
<img src="/documentos/assets/testeSusAtualizado.png" width="100%">
<br>
<br>
<sup>Source: Produced by the authors (2024)</sup>
</div>
...
&nbsp;&nbsp;&nbsp;&nbsp;Embora tenham sido testadas duas versões diferentes da nossa aplicação, acreditamos que a média combinada das pontuações de ambas as versões é relevante. Portanto, criamos um gráfico adicional que apresenta os resultados agregados dos seis testes.

<div align="center">
<sub>Figure 02 - Teste Sus - Resultado Geral</sub>
<br>
<br>
<img src="/documentos/assets/testeSusGeral.png" width="100%">
<br>
<br>
<sup>Source: Produced by the authors (2024)</sup>
</div>
...
&nbsp;&nbsp;&nbsp;&nbsp;Nosso aproveitamento foi de 63,67%, um resultado neutro que indica um desempenho regular. Embora o desempenho seja suficiente, ele aponta para a necessidade de melhorias para alcançar um nível de usabilidade mais satisfatório. Observamos uma significativa variação nas notas dos testes, com uma amplitude de 36 pontos, o que dificultou a identificação clara dos aspectos positivos e negativos da aplicação.

&nbsp;&nbsp;&nbsp;&nbsp;Para lidar com essa variação, criamos uma tabela com as respostas dos seis usuários, na qual a opção mais escolhida em cada item foi considerada para o cálculo final. Nos casos em que houve empate, calculamos a média entre as opções mais selecionadas.

<div align="center">
<sub>Figure - Teste Sus - Tabela com todos</sub>
<br>
<br>
<img src="/documentos/assets/resultadoGeralSus.jpeg" width="100%">
<br>
<br>
<sup>Source: Produced by the authors (2024)</sup>
</div>
...
&nbsp;&nbsp;&nbsp;&nbsp;Com base na análise das respostas, concluímos que o site é considerado pouco intuitivo, uma vez que a maioria dos usuários indicou a necessidade de ajuda para utilizar o sistema. Outra opinião preocupante é a percepção de que o site é inconsistente, um ponto amplamente observado entre os testadores.

&nbsp;&nbsp;&nbsp;&nbsp;Embora esses problemas sejam válidos, é importante notar que os testadores não estavam familiarizados com o objetivo da aplicação e não tinham experiência prévia com o CESIM Game, o que pode ter contribuído para a percepção de falta de intuitividade. Portanto, a necessidade de auxílio para navegar pelo site é menos alarmante nesse contexto específico.

&nbsp;&nbsp;&nbsp;&nbsp;No entanto, a questão da inconsistência é mais crítica. Apesar da justificação pela falta de familiaridade dos usuários com o jogo, acreditamos que o design do sistema pode e deve ser mais consistente. Uma melhor organização e uma apresentação mais coerente ajudarão a melhorar a experiência do usuário e a reduzir a sensação de desorganização.

# <a name="c6"></a>6. Conclusões e trabalhos futuros 

### Alcance dos Objetivos

A solução da aplicação web, conforme descrito na seção 2 deste documento, conseguiu atingir vários dos objetivos estabelecidos. Embora tenhamos enfrentado desafios e limitações, principalmente em termos de escopo e organização, conseguimos entregar um MVP funcional que atende aos requisitos básicos. Entre os principais pontos alcançados, destacamos:

- **Funcionalidades Básicas**: Implementação das funcionalidades essenciais, como criação de universos e grupos dentro desses universos, cadastro de cada usuário; login do usuário; *Home Page*(*Team Page*) com visualização dos dados de outros usuários; realização do questionário (*Self Assesment*) sobre *Decision Make Style*; Implementação das funcionalidades da página *About Me* e envio/organização de dados conforme o modelo relacional.
- **Aprendizado e Crescimento**: A equipe aprendeu significativamente ao longo do projeto, enfrentando desafios técnicos e organizacionais que fortaleceram nossas habilidades.

### Pontos Fortes e Pontos a Melhorar

#### Pontos Fortes
- **Funcionalidades Implementadas**: As funcionalidades básicas de criação de universos, cadastro, login, aparição de dados na *Team Page*, criação da bio na página *About Me* e gestão de dados foram implementadas com sucesso.
- **Aprendizado da Equipe**: A equipe demonstrou uma boa curva de aprendizado, superando com sucesso uma série de desafios técnicos ao longo do caminho. Esse progresso foi evidenciado pela capacidade do grupo de resolver problemas complexos e encontrar soluções inovadoras, o que, por sua vez, impulsionou o desenvolvimento de novas habilidades e conhecimentos. Cada obstáculo vencido não apenas reforçou a confiança e a competência da equipe, mas também solidificou seu compromisso com a excelência e a melhoria contínua. O resultado foi um ambiente de trabalho dinâmico, onde a colaboração e o aprendizado se tornaram parte integrante da rotina diária.

#### Pontos a Melhorar
- **Escopo e Organização**: Durante o desenvolvimento do projeto, ocorreram várias reduções de escopo como resultado de uma má organização e planejamento inicial. Essas deficiências no planejamento levaram a decisões precipitadas e, eventualmente, a uma reavaliação das prioridades do projeto. Em consequência, algumas páginas foram desenvolvidas, mas acabaram não sendo utilizadas posteriormente, refletindo a falta de alinhamento inicial com os objetivos do projeto. Esses ajustes de escopo, embora desafiadores, serviram como lições valiosas para a equipe, destacando a importância de um planejamento mais rigoroso e detalhado nas etapas iniciais para evitar desperdícios de recursos e esforços.
- **Gestão de Tempo**: A gestão inadequada do tempo foi um dos principais desafios enfrentados durante o projeto, resultando em prazos cada vez mais apertados à medida que o trabalho progredia. A falta de um cronograma bem definido e a subestimação do tempo necessário para concluir tarefas críticas levaram a uma pressão constante sobre a equipe. Esta situação, por sua vez, exigiu a tomada de decisões difíceis, como a necessidade de cortar funcionalidades planejadas para cumprir os prazos estabelecidos. O impacto dessas falhas na gestão do tempo foi significativo, destacando a importância de uma abordagem mais estruturada e proativa na alocação de recursos temporais em projetos futuros.

### Planos de Ação para Pontos de Melhoria

Com base nos testes de usabilidade realizados, identificamos áreas que precisam de melhorias e desenvolvemos planos de ação correspondentes:

#### Tarefa 3 - Responder Formulário
- **Problema**: Usuários não conseguiram concluir a tarefa devido a problemas de zoom e ausência de feedback após o envio do formulário.
- **Plano de Ação**: Atualizar a interface para garantir que o botão de submit esteja sempre visível, independentemente do zoom. Implementar um feedback claro para o usuário após a submissão do formulário.

### Ideias para Melhorias Futuras

Além dos pontos de melhoria evidenciados nos testes, temos várias ideias para melhorias futuras:

- **Expansão das Funcionalidades**: Reavaliar as funcionalidades que foram removidas do escopo inicial e considerar sua implementação em versões futuras. Esta reavaliação permitirá identificar quais funcionalidades são mais úteis para os usuários e como elas podem agregar valor à aplicação.
- **Melhoria na Interface do Usuário**:Continuar aprimorando a usabilidade e a estética da aplicação é uma prioridade para oferecer uma experiência de usuário mais intuitiva e agradável. Isso inclui realizar testes de usabilidade, obter feedback dos usuários e incorporar as melhores práticas de design.
- **Automatização de Testes**: Implementar testes automatizados para garantir a qualidade e a consistência das futuras atualizações da aplicação. A automatização ajudará a identificar rapidamente bugs e problemas de desempenho, permitindo uma resolução mais rápida e eficaz.
- **Melhor Gestão de Projetos**:  Aplicar as lições aprendidas sobre gestão de tempo e organização para melhorar a eficiência das futuras iterações do projeto. Isso inclui a criação de cronogramas detalhados, a definição clara de metas e a alocação adequada de recursos para evitar os problemas enfrentados anteriormente.
# <a name="c7"></a>7. Referências

<br>CASAROTTO, Camila. Rockcontent. Disponível em:https://rockcontent.com/br/blog/como-fazer-uma-analise-swot/. Acesso em: 1 mai. 2024 <br>

<br>MAGRETTA, Joan. Entendendo Michael Porter: o guia essencial sobre competição e estratégia. Rio de Janeiro: Elsevier, 2012. <br>

<br>HARLEY, Aurora. Nielsen Norman Group. Disponível em:https://www.nngroup.com/articles/persona/. Acesso em: 18 abr. 2024 <br>

<br>GARRETA, Muriel.Interaction Design Foundation. Disponível em:https://www.interaction-design.org/literature/article/user-stories-as-a-ux-designer-i-want-to-embrace-agile-so-that-i-can-make-my-projects-user-centered. Acesso em: 18 abr. 2024 <br>

<br>ALVES, William P. Java para Web - Desenvolvimento de Aplicações. São Paulo: Editora Saraiva, 2015<br>

<br>GUIMARAES, Felipe. Aela. Disponível em:https://aelaschool.com/pt/designdeinteracao/wireframe-o-que-e-como-desenhar/. Acesso em: 22 abr. 2024 <br>

<br>RED HAT, Red hat. Disponível em:https://www.redhat.com/en/about/brand/standards/typography. Acesso em: 24 abr. 2024 <br>

<br>ARTY, David. Chief of Design. Disponível em:https://chiefofdesign.com.br/ui-design-elementos-da-interface-do-usuario/#google_vignette. Acesso em: 29 abr. 2024 <br>

<br>LEVINE, Oscar. Discrete Mathematics: An Open Introduction. 3. ed. [S.l.: s.n.], 2019. Disponível em: https://discrete.openmathbooks.org/dmoi3.html. Acesso em: 17 jun. 2024.<br>

<br>SILBERSCHATZ, Abraham. Sistema de Banco de Dados. São Paulo: Grupo GEN, 2020<br>

<br>DRIBBBLE. Studio Vor. Disponível em: https://dribbble.com/studiovor. Acesso em: 16 mai. 2024.<br>

<br>FLANAGAN, David. JavaScript: o guia definitivo. São Paulo: Grupo A, 2013<br>

<br>VOLPATO. Eliza. Medium .Disponível em: https://medium.com/testr/o-que-%C3%A9-usabilidade-579f9b285d8e. Acesso em: 20 mai. 2024.<br>

<br>NIELSEN, Jackob. Nielsen Norman Group. Disponível em:https://www.nngroup.com/articles/usability-101-introduction-to-usability/. Acesso em: 22 mai. 2024 <br>

<br> MDN WEB DOCS. O que é CSS?. Disponível em: https://developer.mozilla.org/pt-BR/docs/Learn/CSS/First_steps/What_is_CSS. Acesso em: 17 jun. 2024.<br>
